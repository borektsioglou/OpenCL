...
int accelerate_flow(const t_param params, float* cells, int* obstacles, t_ocl ocl, float w1, float w2);
int propagate(const t_param params, float* cells, float* tmp_cells, cl_mem partial_sum, t_ocl ocl);
...
int write_values(const t_param params, float* cells, int* obstacles, float* partial_sums, int work_groups, int wrk_group_size);
...
int main(int argc, char* argv[])
{
  ...
  int num_wrk_groups = (params.nx*params.ny)/wrk_group_size;
  float* h_partial_sums = calloc(sizeof(cl_float), num_wrk_groups*params.maxIters);
  ...
  d_partial_sums = clCreateBuffer(ocl.context, CL_MEM_WRITE_ONLY, sizeof(cl_float) * num_wrk_groups*params.maxIters, NULL, &err);
  checkError(err, "Creating buffer d_partial_sums", __LINE__);

   accelerate_flow(params,cells,obstacles,ocl, w1, w2);
   propagate(params,cells,tmp_cells,d_partial_sums,ocl);
  ...
  for (int tt = 0; tt < params.maxIters; tt++)
  {
      ...
      // propagate(params,cells,tmp_cells,tt,d_partial_sums,ocl);
    ...
    size_t global = params.nx;
     ...
     checkError(err, "enqueueing accelerate_flow kernel", __LINE__);
	  ...
	  err = clSetKernelArg(ocl.propagate, 8, sizeof(cl_int), &tt);
	  checkError(err, "setting propagate arg 9", __LINE__);

	  global = params.nx*params.ny;
	  size_t local = BLKSIZE;
    ...
    // tot_u = 0.0f;
    // for (int i = 0; i < num_wrk_groups; ++i)
    // {
    // 	tot_u += h_partial_sums[i];
    // 	//printf("tot_u   %f \n",tot_u);
    // }
    // av_vels[tt] = tot_u/(float)params.tot_cells;
    
    ocl.swap_cells = ocl.cells;
    ocl.cells = ocl.tmp_cells;
    ocl.tmp_cells = ocl.swap_cells;
#ifdef DEBUG
    printf("==timestep: %d==\n", tt);
  ...
  systim = timstr.tv_sec + (timstr.tv_usec / 1000000.0);
    ...
    CL_TRUE,
    0,
    sizeof(cl_float) * num_wrk_groups*params.maxIters,
    h_partial_sums,
    0, NULL, NULL);
    // for(int tt=0; tt<153; tt++)
   	//   for(int jj=0; jj<num_wrk_groups; jj++)
    // // if(tt <153)
    //   printf("%f %d %d \n", h_partial_sums[tt*jj], tt, jj);
  ...
  write_values(params, cells, obstacles, h_partial_sums,num_wrk_groups,wrk_group_size);
...
int accelerate_flow(const t_param params, float* cells, int* obstacles, t_ocl ocl, float w1, float w2)
{
  ...
  err = clSetKernelArg(ocl.accelerate_flow, 6, sizeof(cl_float), &w1);
  checkError(err, "setting accelerate_flow arg 6", __LINE__);
  err = clSetKernelArg(ocl.accelerate_flow, 7, sizeof(cl_float), &w2);
...
int propagate(const t_param params, float* cells, float* tmp_cells, cl_mem partial_sums, t_ocl ocl)
{
  cl_int err;
  ...
  err = clSetKernelArg(ocl.propagate, 6, sizeof(cl_float)*BLKSIZE, NULL);
  checkError(err, "setting propagate arg 6", __LINE__);
  err = clSetKernelArg(ocl.propagate, 7, sizeof(cl_mem), &partial_sums);
  ...
  for (int ii = 0; ii < params.ny; ii++)
  {
    for (int jj = 0; jj < params.nx; jj++)
    {
      ...
      if (!obstacles[ii * params.nx + jj])
      {
        ...
        {
          local_density += cells[kk*grid_size + ii * params.nx + jj];
        }

        /* x-component of velocity */
        float u_x = (cells[1*grid_size + ii * params.nx + jj]
                      + cells[5*grid_size + ii * params.nx + jj]
                      + cells[8*grid_size + ii * params.nx + jj]
                      - (cells[3*grid_size + ii * params.nx + jj]
                         + cells[6*grid_size + ii * params.nx + jj]
                         + cells[7*grid_size + ii * params.nx + jj]))
                     / local_density;
        /* compute y velocity component */
        float u_y = (cells[2*grid_size + ii * params.nx + jj]
                      + cells[5*grid_size + ii * params.nx + jj]
                      + cells[6*grid_size + ii * params.nx + jj]
                      - (cells[4*grid_size + ii * params.nx + jj]
                         + cells[7*grid_size + ii * params.nx + jj]
                         + cells[8*grid_size + ii * params.nx + jj]))
                     / local_density;
  ...
  *obstacles_ptr = malloc(sizeof(int) * (params->ny * params->nx));
  ...
  for (int ii = 0; ii < params->ny; ii++)
  {
    for (int jj = 0; jj < params->nx; jj++)
    {
      ...
      /* axis directions */
      (*cells_ptr)[grid_size + (ii * params->nx + jj)] = w1;
      (*cells_ptr)[2 * grid_size + ii * params->nx + jj] = w1;
      (*cells_ptr)[3 * grid_size + ii * params->nx + jj] = w1;
      (*cells_ptr)[4 * grid_size + ii * params->nx + jj] = w1;
      /* diagonals */
      (*cells_ptr)[5 * grid_size + ii * params->nx + jj] = w2;
      (*cells_ptr)[6 * grid_size + ii * params->nx + jj] = w2;
      (*cells_ptr)[7 * grid_size + ii * params->nx + jj] = w2;
      (*cells_ptr)[8 * grid_size + ii * params->nx + jj] = w2;
    }
  }
...
float total_density(const t_param params, float* cells)
{
  ...
  for (int ii = 0; ii < params.ny; ii++)
  {
    for (int jj = 0; jj < params.nx; jj++)
    {
      for (int kk = 0; kk < NSPEEDS; kk++)
      {
        total += cells[kk*grid_size + ii * params.nx + jj];
      }
    }
  }
...
int write_values(const t_param params, float* cells, int* obstacles, float* partial_sums, int work_groups, int wrk_group_size)
{
  ...
  for (int ii = 0; ii < params.ny; ii++)
  {
    for (int jj = 0; jj < params.nx; jj++)
    {
      ...
      else
      {
        ...
        u_x = (cells[1*grid_size + ii * params.nx + jj]
               + cells[5*grid_size + ii * params.nx + jj]
               + cells[8*grid_size + ii * params.nx + jj]
               - (cells[3*grid_size + ii * params.nx + jj]
                  + cells[6*grid_size + ii * params.nx + jj]
              ...
              / local_density;
        /* compute y velocity component */
        u_y = (cells[2*grid_size + ii * params.nx + jj]
               + cells[5*grid_size + ii * params.nx + jj]
               + cells[6*grid_size + ii * params.nx + jj]
               - (cells[4*grid_size + ii * params.nx + jj]
                  + cells[7*grid_size + ii * params.nx + jj]
  ...
  {
  	float sum = 0.0f;
  	for (int jj = 0; jj < work_groups; jj++)
  	{
  		sum +=partial_sums[ii*work_groups+jj];
  		
  	}
  	
  	float written_value = sum/(float)params.tot_cells;
